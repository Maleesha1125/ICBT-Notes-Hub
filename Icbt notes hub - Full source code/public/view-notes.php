...
if (isset($_GET['view']) && !empty($_GET['view'])) {
    $file = $_GET['view'];
    $file_path = __DIR__ . '/../uploads/' . basename($file);
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/view-notes-style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
</head>
<body>
<div class="container">
    <div class="view-notes-card">
        <h2><i class="fas fa-book-open"></i> Content for <?php echo htmlspecialchars($module['name']); ?></h2>
        <?php if ($_SESSION['role'] === 'lecturer'): ?>
            <a href="upload-lecturer-content.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Back to Modules</a>
        <?php else: ?>
            <a href="view-modules.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Back to Modules</a>
        <?php endif; ?>
        <hr>
            ...
            <ul class="list-group">
                <?php foreach ($contents as $content): ?>
                    <li class="list-group-item">
                        <h5><i class="fas fa-file-alt"></i> <?php echo htmlspecialchars($content['title']); ?></h5>
                        <p><?php echo htmlspecialchars($content['description']); ?></p>
                        <small>
                            <i class="fas fa-user-edit"></i> Uploaded by: <?php
                        ...
                        <small>
                            <i class="fas fa-calendar-alt"></i> Uploaded on: <?php echo date('Y-m-d', strtotime($content['upload_date'])); ?>
                        </small><br>
                        <?php if (!empty($content['reviewed_by'])): ?>
                        <small class="text-muted">
                            <i class="fas fa-history"></i> Last updated on: <?php echo date('Y-m-d', strtotime($content['review_date'])); ?> by: <?php
                        ...
                        <div class="mt-2 content-actions">
                            <?php if ($content['content_type'] === 'link'): ?>
                                <a href="<?php echo htmlspecialchars($content['file_path']); ?>" target="_blank" class="btn btn-primary btn-sm"><i class="fas fa-external-link-alt"></i> Go to Link</a>
                            <?php else: ?>
                                <a href="view-notes.php?view=<?php echo urlencode(basename($content['file_path'])); ?>" target="_blank" class="btn btn-primary btn-sm"><i class="fas fa-eye"></i> View</a>
                                <a href="../uploads/<?php echo htmlspecialchars(basename($content['file_path'])); ?>" class="btn btn-success btn-sm" download><i class="fas fa-download"></i> Download</a>
                            <?php endif; ?>
                            <?php if ($_SESSION['role'] === 'lecturer' && $is_lecturer_assigned): ?>
                                <a href="manage-content.php?program_id=<?php echo $module['program_id']; ?>&module_id=<?php echo $module_id; ?>&action=edit&content_id=<?php echo $content['id']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>
                                <a href="#" class="btn-danger" onclick="showDeleteModal('<?php echo $content['id']; ?>', '<?php echo htmlspecialchars(addslashes($content['title'])); ?>', '<?php echo $module['program_id']; ?>', '<?php echo $module_id; ?>')"><i class="fas fa-trash-alt"></i> Delete</a>
                            <?php endif; ?>
                        </div>
                    </li>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i> No notes available for this module yet.
            </div>
        <?php endif; ?>
    </div>
</div>
