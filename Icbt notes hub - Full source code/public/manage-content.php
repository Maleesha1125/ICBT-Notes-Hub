...
<body>
<div class="container">
    <div class="manage-content-card">
        <h2><i class="fas fa-edit"></i> Manage My Content</h2>
        <hr>
            ...
            <?php foreach ($assigned_modules as $module): ?>
                <div class="module-card">
                    <div class="module-header">
                        <h4><i class="fas fa-book-open"></i> <?php echo htmlspecialchars($module['name']); ?></h4>
                    </div>
                    <div class="module-body">
                                ...
                                <tbody>
                                    <?php foreach ($module_contents as $content): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="#" class="btn btn-edit" onclick="showEditModal('<?php echo $content['id']; ?>', '<?php echo htmlspecialchars(addslashes($content['title'])); ?>', '<?php echo htmlspecialchars(addslashes($content['description'])); ?>', '<?php echo htmlspecialchars(addslashes($content['content_type'])); ?>', '<?php echo htmlspecialchars(addslashes($content['file_path'])); ?>', '<?php echo $module['id']; ?>')"><i class="fas fa-pencil-alt"></i> Edit</a>
                                                <a href="#" class="btn btn-delete" onclick="showDeleteModal('<?php echo $content['id']; ?>', '<?php echo htmlspecialchars(addslashes($content['title'])); ?>', '<?php echo $module['id']; ?>')"><i class="fas fa-trash-alt"></i> Delete</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                        ...
                        <?php else: ?>
                            <div class="alert alert-info"><i class="fas fa-info-circle"></i> No content available for this module yet.</div>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <div class="alert alert-warning mt-4">
                <i class="fas fa-exclamation-triangle"></i> You are not assigned to any modules.
            </div>
        <?php endif; ?>
    </div>
</div>
...
<div id="editModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <form id="editForm" action="manage-content.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="edit_upload_type"><i class="fas fa-file-alt"></i> Content Type:</label>
            ...
            <div class="form-group" id="edit_file_group">
                <label for="edit_file"><i class="fas fa-file-upload"></i> File (Leave blank to keep existing file):</label>
                <input type="file" name="file" id="edit_file" class="form-control-file">
            </div>
            <div class="form-group" id="edit_url_group" style="display:none;">
                <label for="edit_url"><i class="fas fa-link"></i> URL:</label>
                <input type="url" name="url" id="edit_url" class="form-control">
            </div>
            <button type="submit" class="btn btn-primary"><i class="fas fa-check-circle"></i> Update Content</button>
            <button type="button" class="btn btn-secondary" onclick="closeEditModal()"><i class="fas fa-times-circle"></i> Cancel</button>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <form id="deleteForm" action="manage-content.php" method="POST">
            ...
            <p><i class="fas fa-exclamation-triangle"></i> Are you sure you want to delete <strong id="deleteContentTitle"></strong>?</p>
            <button type="submit" class="btn btn-delete"><i class="fas fa-trash-alt"></i> Confirm Delete</button>
            <button type="button" class="btn btn-secondary" onclick="closeDeleteModal()"><i class="fas fa-times-circle"></i> Cancel</button>
        </form>
    </div>
</div>
