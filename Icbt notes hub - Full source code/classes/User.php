...
class User {
    ...
    public function registerUser($username, $password, $firstname, $lastname, $role, $department_id, $program_id, $batch, $mobile) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $query = "INSERT INTO users (username, password, firstname, lastname, role, department_id, program_id, batch, mobile, registration_method, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'self', 'inactive')";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("sssssiiis", $username, $hashed_password, $firstname, $lastname, $role, $department_id, $program_id, $batch, $mobile);
        return $stmt->execute();
    }
    ...
    public function addUserManually($firstname, $lastname, $username, $email, $password, $role, $department_id, $program_id, $mobile) {
        ...
        $stmt->bind_param("ssssssiissi", $firstname, $lastname, $username, $email, $hashed_password, $role, $department_id, $program_id, $status, $first_login, $mobile);
    ...
    public function toggleStatus($user_id, $new_status) {
